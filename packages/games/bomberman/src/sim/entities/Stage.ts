...
export class Stage {
  ...
  public collisionMap: CollisionTile[][];

  /**
   * Creates an instance of Stage and initializes the stage map.
   */
  constructor(private readonly data: StageData = stageData) {
    this.tileMap = this.data.tiles.map((row) => [...row]);
    this.collisionMap = this.data.tiles.map((row) =>
